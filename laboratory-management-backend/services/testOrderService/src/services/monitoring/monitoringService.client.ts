...
class MonitoringServiceClient {
  ...
  async createEventLog(payload: MonitoringEventLogPayload): Promise<void> {
    ...
    try {
      ...
      console.error(`[MonitoringServiceClient] Failed to send event log to ${url}:`, error.message);
      if (error.response) {
        console.error("[MonitoringServiceClient] Response data:", error.response.data);
      }
      throw error;
    }
  }
