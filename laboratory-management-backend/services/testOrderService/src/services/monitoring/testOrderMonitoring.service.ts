...
class TestOrderMonitoringService {
  ...
  async recordTestOrderCreated(payload: BaseMonitoringPayload): Promise<void> {
    await this.sendEvent({
      ...payload,
      eventCode: MonitoringEventCodes.TEST_ORDER_CREATED,
      action: MonitoringEventActions.CREATE,
    });
  }

  async recordTestOrderUpdated(payload: BaseMonitoringPayload): Promise<void> {
    await this.sendEvent({
      ...payload,
      eventCode: MonitoringEventCodes.TEST_ORDER_UPDATED,
      action: MonitoringEventActions.UPDATE,
    });
  }

  async recordTestOrderDeleted(payload: BaseMonitoringPayload): Promise<void> {
    await this.sendEvent({
      ...payload,
      eventCode: MonitoringEventCodes.TEST_ORDER_DELETED,
  ...
  private async sendEvent(
    ...
    try {
      ...
      console.error(
        `[TestOrderMonitoringService] Failed to record test order event ${payload.eventCode}: ${errorMessage}`
      );
    }
  }
...
export default new TestOrderMonitoringService();
