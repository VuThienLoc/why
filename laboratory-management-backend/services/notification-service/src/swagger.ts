...
const doc = {
  ...
  host: process.env.NODE_ENV === 'production' ? process.env.RENDER_EXTERNAL_HOSTNAME || process.env.HOST : `localhost:${process.env.PORT || 4002}`,
  basePath: '/api',
  schemes: ['http', 'https'],
  tags: [
    {
      name: 'Notification Service',
      description: 'Notification service operations (requires authentication)'
    }
  ],
  securityDefinitions: {
    apiKeyAuth: {
